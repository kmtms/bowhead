...
do
    ...
    winsfs ${final_name}.saf.idx > ${final_name}.sfs

    tail -n +2 ${final_name}.sfs > ${final_name}_nohead.sfs

    realSFS saf2theta ${final_name}.saf.idx -outname ${final_name} -sfs ${final_name}_nohead.sfs -P 10

    thetaStat do_stat ${final_name}.thetas.idx -win 10000000 -step 10000000 -outnames ${final_name}.thetasWindow.gz
